...
class TestExpandLang:
    def test_language_only(self):
        result = list(_expand_lang("zh"))
        assert result == ["zh"]

    def test_language_and_territory(self):
        result = list(_expand_lang("zh_CN"))
        assert result == ["zh_CN", "zh"]

    def test_language_territory_codeset(self):
        result = list(_expand_lang("zh_CN.UTF-8"))
        assert result == ["zh_CN.UTF-8", "zh_CN", "zh.UTF-8", "zh"]

    def test_language_territory_modifier(self):
        result = list(_expand_lang("zh_CN@collation=pinyin"))
        assert result == ["zh_CN@collation=pinyin", "zh@collation=pinyin", "zh_CN", "zh"]

    def test_language_codeset(self):
        result = list(_expand_lang("zh.UTF-8"))
        assert result == ["zh.UTF-8", "zh"]

    def test_language_modifier(self):
        result = list(_expand_lang("zh@collation=pinyin"))
        assert result == ["zh@collation=pinyin", "zh"]

    def test_full_locale(self):
        result = list(_expand_lang("zh_CN.UTF-8@collation=pinyin"))
        expected = [
            "zh_CN.UTF-8@collation=pinyin",
            "zh_CN@collation=pinyin",
            "zh.UTF-8@collation=pinyin",
            "zh@collation=pinyin",
            "zh_CN.UTF-8",
            "zh_CN",
            "zh.UTF-8",
...
class TestExpandedLangs:
    def test_single_language(self):
        result = list(_expanded_langs(["zh_CN"]))
        assert result == ["zh_CN", "zh"]

    def test_multiple_languages(self):
        result = list(_expanded_langs(["zh_CN", "en_US"]))
        assert result == ["zh_CN", "zh", "en_US", "en"]

    def test_deduplication(self):
        result = list(_expanded_langs(["zh_CN", "zh"]))
        assert result == ["zh_CN", "zh"]

    def test_c_locale_breaks(self):
        result = list(_expanded_langs(["C", "zh_CN"]))
        assert result == []

    def test_c_locale_in_middle(self):
        result = list(_expanded_langs(["zh_CN", "C", "en_US"]))
        assert result == ["zh_CN", "zh"]
...
class TestGetDefaultLocales:
    def test_language_env_var(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("LANGUAGE", "zh_CN:en_US:de")
...
class TestFind:
    ...
    def test_find_existing_translation(self):
        localedir = self._make_traversable({"zh_CN/LC_MESSAGES/test.mo"})
        result = list(find("test", localedir, ["zh_CN"]))
        assert len(result) == 1

    def test_find_fallback_to_language_only(self):
        localedir = self._make_traversable({"zh/LC_MESSAGES/test.mo"})
        result = list(find("test", localedir, ["zh_CN"]))
        assert len(result) == 1
    ...
    def test_find_multiple_languages(self):
        localedir = self._make_traversable(
            {
                "zh_CN/LC_MESSAGES/test.mo",
                "en_US/LC_MESSAGES/test.mo",
            }
        )
        result = list(find("test", localedir, ["zh_CN", "en_US"]))
        assert len(result) == 2
    ...
    def test_find_c_locale_skipped(self):
        localedir = self._make_traversable({"zh_CN/LC_MESSAGES/test.mo"})
        result = list(find("test", localedir, ["C", "zh_CN"]))
        assert len(result) == 0
...
class TestTranslation:
    ...
    def test_returns_null_translations_when_no_file(self):
        localedir = self._make_traversable({})
        _get_or_create_translation.cache_clear()
        result = translation("test", localedir, languages=["zh_CN"])
    ...
    def test_returns_translation_when_file_exists(self):
        mo_data = self._make_minimal_mo()
        localedir = self._make_traversable({"zh_CN/LC_MESSAGES/test.mo": mo_data})
        _get_or_create_translation.cache_clear()
        result = translation("test", localedir, languages=["zh_CN"])
        assert isinstance(result, GNUTranslations)
    ...
    def test_fallback_chain(self):
        ...
        localedir = self._make_traversable(
            {
                "zh_CN/LC_MESSAGES/test.mo": mo_data,
                "en/LC_MESSAGES/test.mo": mo_data,
            }
        )
        _get_or_create_translation.cache_clear()
        result = translation("test", localedir, languages=["zh_CN", "en"])
        assert isinstance(result, GNUTranslations)
    ...
    def test_custom_class(self):
        ...
        result = translation("test", localedir, languages=["zh_CN"], class_=CustomTranslations)
