...
## 如何导入 Excel 工作簿中的联系人数据

**问题描述：**

希望从 Excel 导入联系人，但应用不支持直接读取 Excel 文件。

**解决方案：**

当前版本仅支持通过手动输入或粘贴的方式添加联系人。从 Excel 导入的步骤如下：

1. 调整 Excel 中的字段顺序，使其与 `姓名 电话 备注` 一致。
2. 选中需要导入的联系人数据并复制（<kbd>Ctrl</kbd>+<kbd>C</kbd>）。
3. 在本应用输入框中粘贴（<kbd>Ctrl</kbd>+<kbd>V</kbd>）。
...
输入的号码未匹配任何已启用的号码检测器。可能是号码格式不正确，也可能是对应的检测器未被勾选。

**解决方案：**

1. 检查菜单 **选项 > 号码格式**，确认需要的地区检测器已勾选。
2. 检查输入的号码格式是否正确。
3. 如果某地区受支持的检测器不完整，或您希望添加新地区的检测器，欢迎通过 [GitHub Issues][issues-github] 或 [Gitee Issues][issues-gitee] 提交反馈。
4. 您也可以自行添加号码检测器，详见 [号码检测器开发指南](../dev/globalization/phone-detector.md)。
...
本应用支持通过编辑 Python 配置文件来扩展号码检测器。详细步骤请参阅 [号码检测器开发](../dev/globalization/phone-detector.md)。完成后可通过 Pull Request 提交更改，贡献给项目。
