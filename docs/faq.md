...
本工具目前内置支持中国（含港澳台地区）的常用电话号码格式。

如果输入的号码不匹配任何内置格式，则会触发此提示。如果您发现某地区受支持的格式不完整，或希望添加新地区的格式，欢迎通过 [GitHub Issues][issues-github] 或 [Gitee Issues][issues-gitee] 提交反馈。

## 如何添加对新地区号码格式的支持？

本工具支持通过编辑 Python 配置文件来扩展号码格式。请查阅[贡献指南](../CONTRIBUTING.zh-CN.md)中的“添加电话格式”章节，了解具体步骤。提交您的更改后，我们将非常乐意在审核后将其合并到主项目中。
