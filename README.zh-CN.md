...
- **智能解析**：智能识别 `姓名 电话 备注` 格式，自动合并分隔符。
- **批量导出**：导出所有联系人为单个 `.vcf` 文件。
- **号码校验**：跳过无效号码，一键定位错误行。
- **辅助编辑**：文本区显示行号，支持一键去引号。
- **本地化**：多语言界面，支持中国（含港澳台）号码识别。
- **轻量便携**：提供免安装 ZIP 应用包（`.pyzw`）
- **全平台兼容**：兼容 Windows、macOS、Linux。
- **高性能处理**： 轻松处理百万级联系人，响应迅速。
- **免费开源**：遵循 Apache License 2.0 协议，无广告、无付费。
